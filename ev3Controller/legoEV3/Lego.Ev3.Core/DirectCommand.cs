...
namespace Lego.Ev3.Core
{
	...
	public sealed class DirectCommand
	{
        ...
        /// <summary>
        /// Waits until current operation with an output port is finished and the port become ready for next operation.
        /// WARNING! If you are turning a motor for specified amount of degrees or rotations and the motor encounters
        /// resistance or a physical limit, the method will continue waiting. And it will not return until the resistance is removed.
        /// </summary>
        /// <param name="ports">Port or ports to wait</param>
public
#if WINRT
		IAsyncAction
#else
        Task
#endif
        WaitOutputReadyAsync(OutputPort ports)
        {
            return WaitOutputReadyAsyncInternal(ports)
#if WINRT
			.AsAsyncAction()
#endif
;
...
Task<bool>
        ...
        internal async Task WaitOutputReadyAsyncInternal(OutputPort ports)
        {
            Command c = new Command(CommandType.DirectReply);
            c.WaitOutputReady(ports);
            await _brick.SendCommandAsyncInternal(c);
        }
